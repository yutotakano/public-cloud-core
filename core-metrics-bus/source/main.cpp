...
)
{
  ...
  while (true)
  {
    try
    {
      ...
      std::cout << "Received first byte: " << std::hex << unsigned(buffer[0])
                << std::dec << std::endl;
      if (buffer[0] != 0x99)
      {
        continue;
      ...
      std::cout << "Received second byte: " << std::hex << unsigned(buffer[0])
                << std::dec << std::endl;
      if (buffer[0] != 0x99)
      {
        continue;
      ...
      // Read the header
      bytes_transferred =
        asio::read(socket, asio::buffer(buffer, header_length));
      if (bytes_transferred == 0)
      {
        std::cout << "0 bytes transferred" << std::endl;
        continue;
      ...
      bytes_transferred =
        asio::read(socket, asio::buffer(buffer, message_length));
      if (bytes_transferred == 0)
      {
        std::cout << "0 bytes transferred" << std::endl;
        continue;
    ...
    while (std::getline(message_stream, line))
    {
      ...
      while (std::getline(line_stream, metric, '|'))
      {
        ...
        else if (metric_name == "amf_message_build_latency")
        {
...
int main(int argc, char *argv[])
{
  ...
  // into NGAP structures
  auto &build_latency_summary =
    prometheus::BuildSummary()
      .Name("amf_message_build_latency")
      .Help("Latency between AMF building NAS responses and packing them into "
            "NGAP structures")
      .Register(*registry);
