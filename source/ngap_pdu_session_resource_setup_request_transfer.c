...
int ngap_handle_pdu_session_resource_setup_response_transfer(uint64_t amf_ue_ngap_id, ogs_pkbuf_t *pkbuf) {
    ...
    int store_result = nas_store_ngap_pdu_session_resource_setup_request_transfer_fetch_prerequisites(&params);
...
int nas_store_ngap_pdu_session_resource_setup_request_transfer_fetch_prerequisites(pdu_session_resource_setup_request_transfer_params_t * params) {
    ogs_info("Storing RAN-side TEID and PDN IP into database");
    ...
    // store the TEID and PDN IP into the database
    int db = db_access(NULL, MME_UE_S1AP_ID, amf_ue_ngap_id_buf.buf, 2, 0, UE_TEID, params->ue_teid, ENB_IP, params->pdn_ip);
    ogs_assert(db == OGS_OK);
